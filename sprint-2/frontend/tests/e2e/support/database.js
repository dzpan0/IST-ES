...
Cypress.Commands.add('createDatabaseInfoForShifts', () => {
  cy.task('queryDatabase', {
    query: 'INSERT INTO ' + ACTIVITY_COLUMNS + generateActivityTuple(1, 'A1', 'Activity with 1 participant limit', firstDay.toISOString(), secondDay.toISOString(),
      fifthDay.toISOString(), 1, 1),
    credentials: credentials,
  });
  cy.task('queryDatabase', {
    query: 'INSERT INTO ' + ACTIVITY_COLUMNS + generateActivityTuple( 2, 'A2', 'Activity with 5 participants limit', firstDay.toISOString(), secondDay.toISOString(),
      fifthDay.toISOString(), 5, 1),
    credentials: credentials,
  });
  cy.task('queryDatabase', {
    query: 'INSERT INTO ' + ACTIVITY_COLUMNS + generateActivityTuple(3, 'A3', 'Suspended Activity', firstDay.toISOString(), secondDay.toISOString(),
      fifthDay.toISOString(), 5, 1, 'SUSPENDED'),
    credentials: credentials,
  });
  cy.task('queryDatabase', {
    query: 'INSERT INTO ' + ACTIVITY_COLUMNS + generateActivityTuple(4, 'A4', 'Suspended Activity', firstDay.toISOString(), secondDay.toISOString(),
      fifthDay.toISOString(), 5, 1, 'REPORTED'),
    credentials: credentials,
  });
});
...
Cypress.Commands.add('createDatabaseInfoForEnrollments', () => {
  ...
  cy.task('queryDatabase', {
    query: "INSERT INTO " + SHIFT_COLUMNS + generateShiftTuple(2, dayAfterTomorrow.toISOString(), dayAfterTomorrow.toISOString(), 1, 1),
    credentials: credentials,
  })
  cy.task('queryDatabase', {
    query: "INSERT INTO " + SHIFT_COLUMNS + generateShiftTuple(3, addDays(now, 3).toISOString(), addDays(now, 3).toISOString(), 1, 1),
    credentials: credentials,
  })
  cy.task('queryDatabase', {
  ...
  cy.task('queryDatabase', {
    query: "INSERT INTO " + SHIFT_COLUMNS + generateShiftTuple(4, tomorrow.toISOString(), tomorrow.toISOString(), 1, 2),
    credentials: credentials,
  })
  cy.task('queryDatabase', {
  ...
  cy.task('queryDatabase', {
    query: "INSERT INTO " + SHIFT_COLUMNS + generateShiftTuple(5, tomorrow.toISOString(), tomorrow.toISOString(), 1, 3),
    credentials: credentials,
  })
  cy.task('queryDatabase', {
    query: "INSERT INTO " + SHIFT_COLUMNS + generateShiftTuple(6, addDays(now, 3).toISOString(), tomorrow.toISOString(), 1, 1),
    credentials: credentials,
  })
  cy.task('queryDatabase', {
    query: "INSERT INTO " + SHIFT_COLUMNS + generateShiftTuple(7, addDays(now, 4).toISOString(), dayAfterTomorrow.toISOString(), 1, 1),
    credentials: credentials,
  })
  cy.task('queryDatabase', {
  ...
  cy.task('queryDatabase', {
    query: "INSERT INTO " + ENROLLMENT_SHIFTS_COLUMNS + " VALUES (5, 4)",
    credentials: credentials,
  })
});
...
Cypress.Commands.add('createDatabaseInfoForParticipations', () => {
  ...
  cy.task('queryDatabase', {
    query: "INSERT INTO " + SHIFT_COLUMNS + generateShiftTuple(3, yesterday.toISOString(), yesterday.toISOString(), 2, 1, "Faro"),
    credentials: credentials,
  })
  cy.task('queryDatabase', {
  ...
  cy.task('queryDatabase', {
    query: "INSERT INTO " + ENROLLMENT_SHIFTS_COLUMNS + " VALUES (1, 3)",
    credentials: credentials,
  })
  cy.task('queryDatabase', {
  ...
  cy.task('queryDatabase', {
    query: "INSERT INTO " + USER_COLUMNS + generateUserTuple(6, "VOLUNTEER", "SCENARIO-VOLUNTEER-6", "VOLUNTEER", "NULL"),
    credentials: credentials,
  })
  cy.task('queryDatabase', {
    query: "INSERT INTO " + AUTH_USERS_COLUMNS + generateAuthUserTuple(6, "DEMO", "demo-volunteer-6", 6),
    credentials: credentials,
  })
  cy.task('queryDatabase', {
    query: "INSERT INTO " + USER_COLUMNS + generateUserTuple(7, "VOLUNTEER", "SCENARIO-VOLUNTEER-7", "VOLUNTEER", "NULL"),
    credentials: credentials,
  })
  cy.task('queryDatabase', {
    query: "INSERT INTO " + AUTH_USERS_COLUMNS + generateAuthUserTuple(7, "DEMO", "demo-volunteer-7", 7),
    credentials: credentials,
  })
  cy.task('queryDatabase', {
    query: "INSERT INTO " + ACTIVITY_COLUMNS + generateActivityTuple(99, "A99", "Capacity-limited activity", dayBeforeYesterday.toISOString(), yesterday.toISOString(),
      tomorrow.toISOString(), 5, 1),
    credentials: credentials,
  })
  cy.task('queryDatabase', {
    query: "INSERT INTO " + SHIFT_COLUMNS + generateShiftTuple(99, tomorrow.toISOString(), tomorrow.toISOString(), 1, 99),
    credentials: credentials,
  })
  cy.task('queryDatabase', {
    query: "INSERT INTO " + ENROLLMENT_COLUMNS + generateEnrollmentTuple(99, 4),
    credentials: credentials,
  })
  cy.task('queryDatabase', {
    query: "INSERT INTO " + ENROLLMENT_SHIFTS_COLUMNS + " VALUES (99, 99)",
    credentials: credentials,
  })
  cy.task('queryDatabase', {
    query: "INSERT INTO " + ENROLLMENT_COLUMNS + generateEnrollmentTuple(100, 5),
    credentials: credentials,
  })
  cy.task('queryDatabase', {
    query: "INSERT INTO " + ENROLLMENT_SHIFTS_COLUMNS + " VALUES (100, 99)",
    credentials: credentials,
  })
});
...
function generateActivityTuple(id, name, description, deadline, start, end, participants, institutionId, state = 'APPROVED') {
  return "VALUES ('"
    + id + "', '"
    + deadline +
...
function generateShiftTuple(id, end_time, start_time, participants, activityId, location = 'Lisbon') {
  return "VALUES (" + id + ", '" + end_time + "', '" + location + "', " + participants + ", '" + start_time + "', " + activityId + ")";
}
