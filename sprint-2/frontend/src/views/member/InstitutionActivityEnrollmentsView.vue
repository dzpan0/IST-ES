<template>
  <v-card class="table">
    ...
    <participation-selection-dialog
      ...
      :enrolled-shifts="enrolledShiftsForParticipation"
...
export default class InstitutionActivityEnrollmentsView extends Vue {
  ...
  enrolledShiftsForParticipation: Shift[] = [];
  currentEnrollmentId: number = -1;
  ...
  async selectParticipant(enrollment: Enrollment) {
    ...
    // Get the shifts the volunteer enrolled in
    this.enrolledShiftsForParticipation = enrollment.shiftIds
      ? this.activity.shifts.filter((s) => enrollment.shiftIds.includes(s.id!))
      : [];

    this.currentEnrollmentId = enrollment.id || -1;
  ...
  onCloseParticipationDialog() {
    ...
    this.enrolledShiftsForParticipation = [];
