...
export default class InstitutionActivitiesView extends Vue {
  ...
  getShiftsCapacity(activity: Activity) {
    if (!activity.shifts || activity.shifts.length === 0) {
      return `0 / ${activity.participantsNumberLimit}`;
    }
    const v = activity.shifts.reduce(
        (sum: number, shift: Shift) => sum + (shift.participantsLimit ?? 0), 0
      );
      return `${v} / ${activity.participantsNumberLimit}`;
  }
}
</script>
