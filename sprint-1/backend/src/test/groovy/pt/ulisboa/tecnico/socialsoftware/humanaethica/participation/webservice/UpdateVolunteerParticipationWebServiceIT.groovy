...
class UpdateVolunteerParticipationWebServiceIT extends SpockTest {
    ...
    def setup() {
        ...
        def shift = createShift(activity, SHIFT_LIMIT_3, NOW.minusDays(4).plusHours(1), NOW.minusDays(3).minusHours(1), SHIFT_LOCATION_1)

        def enrollment = new Enrollment()
        enrollment.setEnrollmentDateTime(NOW.minusDays(6))
        ...
        participationService.createParticipation(shift.id, enrollment.getId(), participationDto)
    ...
    def 'log in as another volunteer and try to write a review for a participation by a different volunteer'() {
        ...
        def volunteer = createVolunteer(USER_3_NAME, USER_3_USERNAME, USER_3_EMAIL, AuthUser.Type.NORMAL, User.State.APPROVED)
        volunteer.authUser.setPassword(passwordEncoder.encode(USER_3_PASSWORD))
        userRepository.save(volunteer)
