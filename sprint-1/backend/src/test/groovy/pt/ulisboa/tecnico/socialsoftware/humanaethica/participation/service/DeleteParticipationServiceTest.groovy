...
class DeleteParticipationServiceTest extends SpockTest {
    ...
    def setup() {
        ...
        shift = createShift(activity, SHIFT_LIMIT_2, ONE_DAY_AGO.plusHours(1), NOW.minusHours(1), SHIFT_LOCATION_1)
        otherShift = createShift(activity, SHIFT_LIMIT_2, ONE_DAY_AGO.plusHours(2), NOW.minusHours(2), SHIFT_LOCATION_1)
        ...
        otherEnrollment.setVolunteer(volunteer);
        otherEnrollment.setShifts([otherShift])
        enrollmentRepository.save(otherEnrollment)

        participation = createParticipation(volunteer, enrollment, shift, participationDto)
    ...
    def 'two participations exist and one is deleted: participationId=#participationId | deletedRating=#deletedRating | remainingRating=#remainingRating '() {
        ...
        createParticipation(volunteer2, otherEnrollment, otherShift, participationDto2 )
        firstParticipation = participationRepository.findAll().get(0)
