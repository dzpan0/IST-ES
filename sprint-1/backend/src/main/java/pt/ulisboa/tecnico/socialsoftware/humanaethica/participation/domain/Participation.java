...
public class Participation {
    ...
    public void setShift(Shift shift) {
        this.shift = shift;
        this.shift.addParticipation(this);
    }
    ...
    private void setEnrollmentParticipation() {
        this.enrollment.setParticipation(this);
    }

    private void verifyInvariants() {
        participateOnce();
        numberOfParticipantsLessOrEqualShiftLimit();
    ...
    private void participateOnce() {
        if (this.shift.getParticipations().stream()
    ...
    private void numberOfParticipantsLessOrEqualShiftLimit() {
        if (this.shift.getParticipations().size() > this.shift.getParticipantsNumberLimit()) {
    ...
    private void shiftReferenceCheck() {
        if (this.enrollment.getShifts().stream().noneMatch(shift -> shift == this.shift)) {
            throw new HEException(ErrorMessage.PARTICIPATION_SHIFT_NOT_IN_ENROLLMENT);
        }
    }
}
