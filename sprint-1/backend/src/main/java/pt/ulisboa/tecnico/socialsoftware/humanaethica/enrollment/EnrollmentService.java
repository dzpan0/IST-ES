...
public class EnrollmentService {
    ...
    private List<Shift> getShifts(EnrollmentDto enrollmentDto) {
        List<Shift> shifts = new ArrayList<>();
        enrollmentDto.getShifts().stream().forEach(shiftDto -> {
            if (shiftDto.getId() == null)
                throw new HEException(SHIFT_NOT_FOUND, shiftDto.getId());
            Shift shift = shiftRepository.findById(shiftDto.getId()).orElseThrow(() -> new HEException(SHIFT_NOT_FOUND, shiftDto.getId()));
            shifts.add(shift);
        });
        return shifts;
    }
    ...
    public EnrollmentDto createEnrollment(Integer userId, EnrollmentDto enrollmentDto) {
        ...
        if (enrollmentDto.getShifts() == null) throw  new HEException(ENROLLMENT_NO_SHIFTS);
        List<Shift> shifts = getShifts(enrollmentDto);
